...
//		ServoData *servo_data = malloc(servo_count * sizeof(ServoData));
    ...
    // 发送请求包
		if(size<=255)    FSUS_SendPackage_Common(usart, FSUS_CMD_SET_SERVO_SyncCommand,(uint8_t)size, buffer,0);
		else     FSUS_SendPackage_Common(usart, FSUS_CMD_SET_SERVO_SyncCommand,size, buffer,1);
