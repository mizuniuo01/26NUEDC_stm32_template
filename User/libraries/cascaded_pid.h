...
#define AUTO_BALL_CAR_USER_LIBRARIES_CASCADED_PID_H /* 头文件保护 */
...
/* 串级 PID 公共接口状态码 */
typedef enum {
    CASCADED_PID_STATUS_OK = 0,               /* 操作成功 */
    CASCADED_PID_STATUS_INVALID_ARGUMENT = 1, /* 参数为空或取值非法 */
    CASCADED_PID_STATUS_INVALID_CONFIG = 2,   /* 控制器配置不满足约束 */
    CASCADED_PID_STATUS_NOT_INITIALIZED = 3,  /* 控制器尚未初始化 */
...
typedef struct {
    ...
    bool is_angle_enabled;         /* 航向外环启用标志 */
...
typedef struct {
    ...
    bool is_angle_initialized;     /* 航向展开器是否已经初始化 */
    bool is_initialized;           /* 实例是否已经初始化 */
} cascaded_pid_t;
