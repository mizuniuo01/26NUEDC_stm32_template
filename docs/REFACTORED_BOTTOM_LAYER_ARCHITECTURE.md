...
步进电机发送接口保留原版的角度、速度、加速度、三种位置模式和同步标志语义。当前电机已使能
`S_PosTDP` 通讯位置精度，线上 16 字节帧为：地址、`0xFD`、方向、加速度、减速度、速度、以
0.01° 为单位（角度放大 100 倍）的大端位置数值、模式、同步标志和 `0x6B`。USART2 已由
CubeMX 配置 DMA 收发，命令发送使用 TX DMA；驱动还会解析 `0x36` 返回的 8 字节实时位置帧。
